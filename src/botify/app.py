...
function renderDiagnostics(diagnostics) {
  ...
  document.getElementById('diagnostics').innerHTML = [
    ...
    ['Profit Factor', diagnostics.profit_factor === null ? 'infinite' : num(diagnostics.profit_factor), diagnostics.profit_factor >= 1 ? 'good' : 'bad'],
    ['Best Trade', money(diagnostics.biggest_win), pnlClass(diagnostics.biggest_win)],
    ['Worst Trade', money(diagnostics.biggest_loss), pnlClass(diagnostics.biggest_loss)],
...
def _diagnostics_payload(snapshot: dict) -> dict:
    ...
    target_distances = [abs(position["target_price"] - price) / price * 100 for position in positions if price]
    return {
...
def trades_csv() -> str:
    ...
    writer.writerow(["side", "entry_price", "exit_price", "quantity", "pnl", "reason", "opened_at", "closed_at"])
    for trade in trades:
        writer.writerow([trade.side, trade.entry_price, trade.exit_price, trade.quantity, trade.pnl, trade.reason, trade.opened_at, trade.closed_at])
