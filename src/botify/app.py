...
function renderDashboard(data) {
  ...
  document.getElementById('settings').innerHTML = [
    ...
    ['Max Pending Orders', data.config.max_pending_orders],
    ['Max Pending / Side', data.config.max_pending_orders_per_side],
    ['Max Total Notional', `${num(data.config.max_total_notional_pct * 100)}% equity`],
    ['Entry Offset', `${num(data.config.passive_entry_offset_steps)} grid steps`],
...
function renderSafetyGuardrails(data) {
  ...
  const cards = [
    ...
    [
      'Pending Slots',
      `${plan.pending_slots_used || 0} used / ${plan.pending_slots_remaining || 0} left`,
      (plan.pending_slots_remaining || 0) ? 'good' : 'warn',
    ],
    [
      'Pending Long / Short',
      `${plan.long_pending_orders || 0} / ${plan.short_pending_orders || 0}`,
      '',
    ],
...
def _diagnostics_payload(snapshot: dict) -> dict:
    ...
    return {
        ...
        "pending_slots_used": grid_plan.get("pending_slots_used", 0),
        "pending_slots_remaining": grid_plan.get("pending_slots_remaining", 0),
